...
**The v1→v2 migration is complete** — all 7 phases below are done, including deployment (Render + Neon + Vercel) and the README rewrite, both verified end-to-end in Chrome and Safari. The plan below (designed and approved 2026-08-15) remains durable project context, not a one-off session note — it's the record of *why* things are built the way they are, still worth reading before touching auth, sharing, or the data model. Only the "Open items needing the user" section (Google consent screen publishing) and any genuinely new feature work remain. Continue updating the Progress checklist and phase implementation notes if that happens.
...
### Phase 4 implementation notes (settled while wiring frontend auth, not in the original design)

- **No React Context Provider needed for auth.** `better-auth/react`'s `createAuthClient()` hooks (`useSession()`, etc.) are backed by a module-level store, not React context — any component can call `authClient.useSession()` directly by importing the shared instance from `frontend/src/shared/api/auth-client.ts`. `App.tsx` needed zero changes for this.
- **Vite wasn't reading the repo's `.env` at all.** `vite.config.ts` sets `root: "./frontend"`, and Vite defaults `envDir` to `root` — so `VITE_API_URL` and friends would silently resolve to `undefined` even though `.env` at the repo root has them. Fixed by explicitly setting `envDir: import.meta.dirname` (the repo root) in `vite.config.ts`. Worth remembering if a `VITE_*` var ever seems to "not exist" despite being in `.env`.
- **`VITE_API_URL` stays empty in local dev on purpose.** Both `shared/api/client.ts` and `shared/api/auth-client.ts` fall back to relative/same-origin requests when it's unset, relying on the Vite dev proxy (`vite.config.ts`, `/api` → `NEST_PORT`) to reach the backend. This means the browser never sees the request as cross-origin locally, which sidesteps needing `sameSite:"none"; secure:true` cookies in dev entirely (those only get set in production — see `server/src/auth/auth.ts`'s `isProduction` branch). Only set `VITE_API_URL` for a genuinely separate-origin deployment.
- **`loginSchema`/`signupSchema` aren't paired to a backend DTO** via `satisfies z.ZodType<...>` the way the other shared schemas are — the auth forms POST directly to Better Auth's own `/sign-in/email` and `/sign-up/email` endpoints, which validate independently server-side. These schemas exist purely for client-side form UX consistency with the rest of the app.
- **Verification note**: automated in-browser verification (chromium-cli / Playwright) wasn't available in this environment, so the auth flow was confirmed by the user testing directly in their own browser — email/password signup and login both work end-to-end; Google sign-in correctly errors with "provider not found" (expected — no OAuth client configured yet, see Open Items).

### Phase 5 implementation notes (settled while building feature UI, not in the original design)

- **Google logo**: the "Continue with Google" buttons use `frontend/public/google-logo.svg`, supplied directly by the user rather than an inline SVG guessed from memory — more reliable for exact brand-icon path data than reproducing it.
- **`moveFile` is one query, not the three/four a naive ownership-check-then-folder-check-then-update-then-refetch would take.** Prisma allows extra non-unique conditions alongside a unique `id` in `update()`, applied as additional WHERE clauses — so ownership *and* "the target folder exists in this same data room" ride along with the update itself in one round trip, and (unlike `updateMany`) `update()` returns the row. A combined-condition miss surfaces as Prisma error P2025, added as a new `isRecordNotFound()` helper in `common/db-errors.ts` alongside the existing `isUniqueViolation()`. The starred-flag lookup for the response is a targeted single-row check (`starredItem.findFirst`), not the bulk `getStarredIds()` used elsewhere — cheaper for a single-file response. This pattern (one combined `update`, targeted starred lookup) could also tighten `renameFile`/`getFileById`, which still use the older `updateMany` + `findUniqueOrThrow` + bulk `getStarredIds` shape — not changed yet since it wasn't asked for, but worth doing together if either is touched again.
- **Move conflict UX is a deliberate consequence of a design discussion, not the default plan.** Move hard-rejects on a name collision (409) exactly like rename, *not* like upload's auto-suffix — reasoning: move is a single, watched, one-target action (same category as rename), so silently renaming the file out from under the user would be a surprise, not a convenience. The conflict is shown *inline in the still-open dialog* (`MoveFileDialog`'s `moveError` state), not a toast — the user stays in the folder-tree-picker and can immediately choose a different destination, mirroring `useEntryActions.handleRenameSubmit`'s existing inline-error pattern for the exact same reason.
- **`folder-tree-picker.tsx` ended up inside `features/file-actions/ui/`, not `shared/components/`** as the original plan sketch said. Reason: it needs to fetch folder contents, and this app's Feature-Sliced Design rule is strict about only importing "downward" — `shared/` can't import from any `features/*`, including `dataroom-actions` (which owns `getDataroomContents`). Rather than violate that or promote `getDataroomContents` out of its feature, `file-actions/api/fileApi.ts` got its own thin `listSubfolders()` wrapper hitting the same `/datarooms/:id/contents` endpoint directly via `shared/api/client.ts` — no cross-feature import, no new backend endpoint.
- **Two follow-up refactors landed after the fact, both from the user reviewing the diff, not planned upfront**: (1) `menuItems` (identical `ActionsMenuItem[]` construction) was duplicated between `EntryRow.tsx` and `EntryCard.tsx` — moved into `useEntryActions.ts`, which already centralizes every other piece of shared state between those two views. (2) `useEntryActions.ts` itself was sitting in `pages/dataroom/ui/` despite being a hook, not a component — moved to a new `pages/dataroom/model/` folder, matching the `api/model/ui` split every `features/*` slice already uses. `pages/dataroom/ui/types.ts` was reviewed for the same question and left alone: it holds `ViewMode`, genuinely shared between two components, which is different from a `*Props` interface owned by exactly one component (those stay co-located, matching how the whole codebase already does it — `ActionsMenuProps` in `actions-menu.tsx`, `NameFormDialogProps` in `name-form-dialog.tsx`, etc.).
- **Drag-and-drop needed no backend change** — `UploadDropzone` (`features/upload-file/ui/`) wraps `DataroomPage`'s whole content area (not just the empty-folder state) and calls the exact same `enqueueFiles` the click-to-browse `UploadButton` already used, so both entry points share one upload queue and one `UploadProgressList`. Uses a drag-enter counter (increment on dragenter, decrement on dragleave, show overlay while > 0) rather than checking `dragleave`'s `relatedTarget` — the counter avoids the overlay flickering as the pointer crosses child element boundaries (table rows, cards) while dragging over nested content, a well-known rough edge with the naive approach.
- **Recurring IDE-diagnostic gotcha**: throughout Phase 5, the IDE's inline TS diagnostics repeatedly lagged one edit behind reality — flagging "unused" symbols that were used two edits later, or syntax errors in JSX that was actually balanced. Every one of these was a false alarm once checked against the real `tsc -p server/tsconfig.json` / `tsc -b` output. If an IDE-reported error doesn't reproduce in a real typecheck run, trust the real compiler.
- **`SharedResolver` + `SharedViewLayout` started as two components, ended as one.** First pass split "resolve the token" (a leaf route at `path="shared/:token"`) from "render the branded chrome" (a parent route at the *same* `path="shared/:token"`, matched via react-router's specificity ranking rather than declaration order) — each independently called `useResolveShareToken`, relying on TanStack Query's cache (10s global `staleTime`, set in `query-provider.tsx`) to quietly absorb the duplicate fetch. Caught in review: two routes sharing one path string and two call sites fetching the same thing were both real red flags, not just style nits — merged into a single `SharedViewLayout` that fetches once, handles loading/error itself, and redirects into the fuller path only when `useMatch("/shared/:token")` confirms we're at the bare token URL. One fetch site, one route entry, no reliance on cache behavior to hide redundant work.
- **`BrowseContext` grew a `buildPath()` helper** (`defaultBuildPath` vs. `buildSharedPath(token)`) specifically so the public-link work wouldn't require threading a `token` string through half the component tree (`useEntryActions`, `Breadcrumbs`, `FilePreviewPanel`'s "back" button) just to get link generation right in both modes — those components ask the context for a link instead of string-building `/datarooms/...` themselves.
...
- [x] Phase 4: Frontend auth (login/signup incl. Google, `RequireAuth`, client wiring, user menu) — verified: `npm run typecheck` clean, and the user manually confirmed email/password signup + login work end-to-end in the browser (lands on /datarooms, sidebar user menu shows). Google sign-in correctly shows a "provider not found" error, as expected — no Google Cloud OAuth client is configured yet (still an open item below), and `server/src/auth/auth.ts` only registers the provider when credentials are present.
- [x] Phase 5: Frontend feature UI — done in chunks (user's request), tracked here as each landed:
  ...
  - [x] "Shared with me" page — `pages/shared-with-me/`, mirrors `pages/starred/`'s structure closely (same table shape, empty state, row pattern). Extracted `buildEntryLink()` into `shared/lib/entry-links.ts` since `StarredRow` and the new `SharedWithMeRow` both needed the exact same entityType→route logic — deduplicated instead of copying a third time. Added to the sidebar nav between "Data Rooms" and "Starred". Prompted directly by the user testing sharing manually and finding no way to discover what had been shared with them — this chunk got reprioritized ahead of the public `/shared/:token` link flow (still pending) because it's what closes that gap for logged-in grantees.
  ...
  - [x] Read-only gating Part 1: `isOwner`-based UI gating on the *existing* authenticated routes. `shared/lib/browse-context.tsx` (`BrowseContextProvider`/`useBrowseMode`, defaults `isReadOnly: true` — fails closed), set by `DataroomPage` from `data.isOwner` (defaults to read-only while loading, before isOwner is known). Consumed by `useEntryActions` (empty `menuItems`, including hiding Star — starring is still owner-only server-side, not yet a per-viewer annotation), `Toolbar` (hides New Folder/Upload, and Search too since it's owner-only server-side — see Phase 3 notes), `EmptyFolder` (swaps copy, hides buttons), `FilePreviewPanel` (now reuses `useEntryActions`'s `menuItems` instead of hand-rolling its own — fixed a pre-existing gap where the preview panel's menu was missing Share/Move that EntryRow/EntryCard already had). `ActionsMenu` isn't rendered at all when empty, rather than showing an empty dropdown.
  - [x] Read-only gating Part 2: `GET /shares/token/:token/resolve` (`@AllowAnonymous`, throttled — the one endpoint on `SharingController` reachable with no session), `BrowseContext` extended with a `buildPath()` helper (`defaultBuildPath` for `/datarooms/...`, `buildSharedPath(token)` for `/shared/:token/datarooms/...` — components ask for a link instead of string-building one, so `DataroomPage`/`useEntryActions`/`Breadcrumbs`/`FilePreviewPanel` all work correctly in both modes unchanged otherwise), and `SharedViewLayout` as the branded chrome + token resolver in one component (see implementation notes — a first pass had this split into two components each independently fetching the same resolve call, caught in review and merged into one). `DataroomPage`/`FilePreviewPanel` now read an optional `token` route param alongside `dataroomId`/`folderId` and thread it into `useDataroomContents`/`useFile`, both already token-aware since Phase 3. Verified end-to-end via curl: a folder-scoped public link resolves to that folder, anonymous fetch of its contents succeeds, and anonymous fetch of the dataroom *root* (outside the shared subtree) correctly 404s.
  - [x] Bulk select + bulk delete/move — backend: `FoldersService.bulkSoftDelete` (per-id ownership check, then each folder's existing per-folder transaction runs independently — not one giant cross-subtree transaction, since selected folders are logically independent), `FilesService.bulkDelete` (single `updateMany`), `FilesService.bulkMove` (loops the already-optimized single-file `moveFile`, best-effort: a per-file 409 is counted in `conflictCount` and skipped rather than failing the whole batch, matching move's hard-reject convention — not upload's auto-suffix). All three ids-not-owned-or-missing are silently excluded, matching existing single-item 404 conventions. `POST /folders/bulk-delete`, `POST /files/bulk-delete`, `PATCH /files/bulk-move` — the last one declared *before* the pre-existing `PATCH /files/:id` route, since Nest/Express matches by declaration order (not specificity, unlike react-router), so the `:id` wildcard would otherwise swallow the literal `bulk-move` path. 4 new backend tests (`server/src/test/bulk-actions.test.ts`). Frontend: `useBulkSelection` (`pages/dataroom/model/`) lifts a `Map<"type-id", BrowserEntry>` selection into `DataroomPage`, cleared on dataroom/folder/search change so stale off-screen ids can never get bulk-acted-on; `EntryTable`/`EntryGrid` gained a checkbox column/corner (hidden in read-only mode, same gating as the rest of the mutating UI) and a `SelectionToolbar` swaps in for the normal `Toolbar` while ≥1 item is selected. `BulkMoveDialog` (files-only, reuses `FolderTreePicker`) lives inside `features/file-actions/` since it only touches that one feature; `BulkDeleteDialog` (folders + files) lives at the page layer since it composes both `folder-actions` and `file-actions` bulk mutations, same reasoning as `useEntryActions.ts`. Move-to-… is disabled with an explanatory tooltip when the selection includes any folders (bulk-move stayed files-only, per the original plan). Verified manually by the user: bulk move and bulk delete (mixed folders + files) both work as expected.
  - [x] Owner-facing activity panel — dataroom-wide, not per-resource: `GET /datarooms/:id/activity` (new `ActivityController`/`ActivityService`/`ActivityModule`, sharing the `datarooms` path prefix with `DataroomsController`) reads `AccessLog` rows for the dataroom itself plus every folder/file inside it, ordered most-recent-first and capped at 50, enriched with the viewer's current name/email (or `null` = anonymous-via-public-link) and the current resource name (or `null` if the resource has since been permanently purged — `AccessLog` has no FK/cascade to folders/files by design, so history survives a purge). Scoped to one dataroom rather than one endpoint per resource/one endpoint for the whole account, since "who's been looking at *this* data room" is the actual owner question and it avoids denormalizing a `dataroomId` column onto `AccessLog` just for this. 3 new backend tests (`server/src/test/activity.test.ts`), including a regression check (from the read side, mirroring the write-side invariant in `shares-access.service.ts`) that the owner's own views never appear. Frontend: new `features/activity/` slice, `ActivityPanel` as a right-side `Sheet` (shadcn's, previously only used internally by the mobile sidebar) opened via an "Activity" button in `Toolbar` — owner-only, hidden in read-only mode like the rest of the mutating/owner UI. Each row: viewer name/email or "Anonymous" + "via public link", what was viewed, relative timestamp.
- [x] Phase 6: Deployment — **Render (free web service) + Neon (free Postgres) + Vercel (static frontend)**, not Railway (trial expired) and not Render's own Postgres (free tier auto-deletes after 30 days; Neon's free tier doesn't expire). Verified end-to-end in both Chrome and Safari: signup, email/password login, Google OAuth login/signup, dataroom/folder/file CRUD, upload, sharing (including a public link opened anonymously), and activity — all working against the real deployed stack. See "Phase 6 implementation notes" below for the real, non-obvious issues hit and fixed along the way (a `render.yaml` free-tier constraint, an `npm ci` lockfile-determinism bug, an auth/CORS bug in the upload flow, a pathname-trim mismatch, a wasted-but-harmless 404, and — the biggest one — Safari's cross-site cookie blocking plus Chrome's Safe Browsing flag on `onrender.com`, both fixed by proxying `/api/*` through Vercel).
- [x] Phase 7: README — full rewrite (v1's was entirely Express/Drizzle/no-auth, now stale): live demo URLs, updated tech stack, setup instructions for the v2 stack (Prisma migrate, Better Auth env vars, optional Google OAuth setup with the frontend-origin redirect URI gotcha called out), project structure, a Mermaid ERD (renders natively on GitHub) with the `Share`-uses-three-FKs and `AccessLog`-has-no-FKs design calls explained, all three "how it scales" answers (subtree stats via the existing recursive-CTE id set + aggregates; 100k-file changes: keyset pagination, composite indexes matching actual query shapes, `pg_trgm`/real search for filename search past `ILIKE`, denormalized dashboard stats; per-user roles via the already-commented `Share.role` additive column + an `assertCanEdit` reusing the same ancestor-chain resolution), design-decision writeups including the two production-only bugs and their fixes (Safari ITP → Vercel proxies `/api`; Google OAuth redirect → per-provider `redirectURI` through the same proxy), the "beyond the brief" list, an updated scope-cut list, and an AI-usage note describing the actual collaboration (plan-first, human review catching and redirecting real issues, reproduction-driven production debugging) rather than a generic one. `user`/`session`/`account`/`verification` singular-table-name note folded into the ERD section instead of a standalone item.

### Phase 6 implementation notes (settled while deploying, not in the original design)

- **Render's free plan doesn't support `preDeployCommand`.** `render.yaml`'s migration step (`prisma migrate deploy`) had to move into `buildCommand` instead (`npm ci --include=dev && npm run prisma:migrate:deploy && npm run build:nest`) — harmless since `migrate deploy` is idempotent (no-ops if already up to date), so re-running it on every build costs nothing.
- **`npm ci` needs `--include=dev` on Render.** `NODE_ENV=production` (set for the Better Auth cookie branch, see Phase 1) also makes npm omit devDependencies during install unless told otherwise — `typescript`/`prisma` happened to survive anyway (pulled in transitively by something in `dependencies`), which masked the omission at first; `@types/express` (a pure, nothing-else-needs-it devDependency) didn't, surfacing as a `TS7016` "could not find a declaration file for module 'express'" build failure.
- **A real `package-lock.json` drift, unrelated to the above, then blocked `npm ci` outright** (`EUSAGE`, lockfile out of sync) — traced to `ajv-formats`: `@modelcontextprotocol/sdk` (pulled in transitively via `shadcn`, itself only a dev-time CLI tool) hard-requires `ajv-formats@^3.0.1`, while `@hookform/resolvers` merely lists it as an **optional, unused** peer wanting `^2.1.1` (this app only uses zod resolvers, never ajv). `npm install` tolerates the ambiguity; `npm ci` doesn't, and different npm versions resolved it differently between this machine and Render's. Fixed with `"overrides": { "ajv-formats": "^3.0.1" }` in `package.json`, pinning it to the version something *actually* hard-requires — eliminates the ambiguity outright rather than picking a side per-environment.
- **File upload broke in production two ways, both from the same root design flaw.** `@vercel/blob/client`'s `upload()` convenience helper makes its own internal `fetch()` to `handleUploadUrl` with no way to set `credentials: "include"` — fine same-origin (worked in dev), but the session cookie never attaches once frontend/backend are genuinely cross-origin, so the backend's `AuthGuard` 401s. Fixed by splitting into two steps: `blobService.generateUploadToken()` now uses `generateClientTokenFromReadWriteToken` directly (a lower-level function, not `handleUpload`) and is called through the app's own authenticated `api` client first, then the browser's own `put()` (also from `@vercel/blob/client`, taking a pre-obtained token) performs the actual upload straight to Blob storage — that leg never touches the backend, so it was never the cookie problem. Second bug surfaced once real filenames were tested: the backend's `generateUploadTokenSchema` trims whitespace but the frontend was calling `put()` with the raw untrimmed `file.name` — a token generated for a trimmed pathname doesn't match a PUT for the untrimmed one, and Vercel Blob storage hard-rejects the whole request (`"pathname" ... does not match the token payload`, HTTP 403) rather than silently reconciling it. Fixed by trimming once in `uploadFileApi.ts` and reusing that exact string for the token request, the PUT, and the confirm call.
- **A file-level public share always 404'd on one background request, but never actually broke anything visible.** `DataroomPage` unconditionally fetches the current folder's contents (for breadcrumbs), even while a file is being previewed via `<Outlet/>` — and for a file-scoped token, that fetch is *guaranteed* to 404 by design (the token only grants access to that one file, not its parent folder's listing — see `shares-access.service.ts`). `FilePreviewPanel` never actually uses that data (it fetches the file itself independently, also token-aware), so this was pure waste against the public endpoint's rate limit, not a real bug. Fixed by skipping the fetch (`useDataroomContents`'s new `enabled` param) specifically when `isPreviewingFile && token` — narrower than "skip during any file preview," since the authenticated (no-token) case still needs that same fetch for `isOwner`/`isReadOnly`, which `FilePreviewPanel`'s own mutating menu depends on.
- **The core production blocker: Safari blocks the cross-site session cookie; Chrome's Safe Browsing separately flags `onrender.com`.** Frontend (Vercel) and backend (Render) are different sites, so every `fetch(..., {credentials:'include'})` from the frontend is third-party from Safari's ITP perspective — email/password login would succeed (200, session token in the body) but the immediately-following `getSession` request came back `null`, since Safari never stored the `Set-Cookie` in the first place. Chrome doesn't enforce this (yet), so the same flow worked there — the two browsers disagreeing was the tell. Fixed at the root, not by fighting Safari's cookie policy: **`vercel.json` now proxies `/api/*` to the Render backend** (`"destination": "https://<render-service>.onrender.com/api/:path*"`), and `VITE_API_URL` was removed from Vercel's env vars so the frontend goes back to relative/same-origin requests (the exact same mechanism local dev already used via Vite's proxy — see Phase 4 notes). The browser now only ever talks to one origin; Vercel forwards transparently including `Set-Cookie`, so cookies land as ordinary same-origin cookies with no ITP involvement at all.
- **Google OAuth needed the same treatment, via a different mechanism.** Google's redirect is a real top-level navigation straight to whatever `redirect_uri` Better Auth constructs from its own `baseURL` (Render) — bypassing the Vercel proxy entirely regardless of the fix above, since Google doesn't know or care about it. Two separate symptoms: Chrome's Safe Browsing outright blocked the navigation to `onrender.com` (`Небезпечна` / dangerous-site warning — a reputation issue with Render's shared free-tier domain, not something fixable in-app), and Safari threw `state_mismatch` (the OAuth state cookie, now set on Vercel's origin during sign-in initiation since `authClient`'s `baseURL` is also unset, could never be read back on Render's completely different origin during the callback). Fixed with Better Auth's **per-provider `redirectURI` override** (`socialProviders.google.redirectURI`, not the global `baseURL`) pointed at `${FRONTEND_ORIGIN}/api/auth/callback/google` — Google now redirects to Vercel, which proxies the callback through to Render exactly like every other request, so initiation and callback share one origin throughout. Required updating Google Cloud Console's Authorized redirect URIs to match (`.../api/auth/callback/google` on the frontend origin, not the backend one — and note the local one changes port too, from `:3001` to `:5173`).
- **`account_not_linked`**: signing in with Google using an email that already had an email/password account (unverified, since this app never verifies email/password signups at all — see Phase 1) threw this instead of linking the two accounts, since Better Auth's default auto-link path requires the existing account's email to already be verified. Fixed with `account.accountLinking.trustedProviders: ["google"]` — reasonable specifically for Google (unlike the general case) since Google itself verifies the email before ever handing it back to us.
- **Leftover local dev server processes repeatedly squatted on port 3001** across many rounds of local verification this session (`node --watch` respawning, `concurrently`'s process tree outliving a plain `lsof -ti:PORT | xargs kill` that only targets whatever's *currently* bound to the port) — `pkill -f "node --watch server/dist/server/src/main.js"` and `pkill -f "concurrently -n tsc,run"` alongside the port-based kill is what actually clears it.
...
- **Google OAuth consent screen is likely still in "Testing" mode**, which restricts sign-in to explicitly-added test-user emails — fine for the account(s) used to verify this, but anyone else (e.g. a reviewer) trying "Continue with Google" cold will hit an access-blocked screen unrelated to the app itself. Either add their email as a test user or publish the consent screen before relying on Google sign-in for an unknown visitor.
