...
- Frontend stays on Vercel, now static-only: remove `api/index.ts` and the `vercel.json` rewrite (`/api/(.*) → /api/index`). Add `VITE_API_URL`; `frontend/src/shared/api/client.ts`'s hardcoded `` `/api${path}` `` becomes `` `${import.meta.env.VITE_API_URL}${path}` ``.
...
- [ ] Phase 2: Core CRUD ported to Nest+Prisma (datarooms/folders/files/trash/starred), existing integration tests ported and passing
- [ ] Phase 3: Sharing backend (`shares-access.service.ts`, `AccessLog`, expiring/strong tokens) + new e2e tests
