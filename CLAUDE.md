...
- [ ] Phase 7: README (ERD, "how it scales" ⓐⓑⓒ, AI-usage note). **Also note in the data-model section**: `user`/`session`/`account`/`verification` tables are singular (Better Auth's own generator convention, confirmed via their docs — not a mistake), while every domain table (`datarooms`, `folders`, etc.) is plural, matching v1's Drizzle convention. Deliberately left as-is rather than force-renamed, since Better Auth would silently revert a manual rename the next time its generator runs. User decision, made 2026-08-17.
